...
public class SummOfTwoDigits {
    ...
    static int[] sumOfPairEnumeration(int[] nums, int k) {
        //входяшие цыфры 4 1 -5 4 6 4 ; 5 1 5 4 -6 4 4 ; 3 -9 5 3 9
        int arrDiff = Integer.MAX_VALUE;
        for (int i = 0; i < nums.length; i++) {
            for (int j = i + 1; j < nums.length; j++) {
                if (nums[i] + nums[j] == k)
                    return new int[]{nums[i], nums[j]};
                else {
                        arrDiff = Math.min(Math.abs(k - (nums[i] + nums[j])), arrDiff);
                    if (i >= nums.length - 2) {
                        for (int l = 0; l < nums.length; l++) {
                            for (int m = l + 1; m < nums.length; m++) {
                                if (Math.abs(k - (nums[l] + nums[m])) == arrDiff)
                                    return new int[]{nums[l], nums[m]};
                            }
                        }
                    }
                }
